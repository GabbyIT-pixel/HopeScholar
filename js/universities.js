...
async load(){
  ...
  try{
    const res=await fetchWithTimeout(`https://universities.hipolabs.com/search?country=${encodeURIComponent(validateInput(country,50))}`);
  ...
  }catch(e){hideLoader('uni');showError('uni',`Could not load universities: ${e.message}`);}
},
render(){
  const grid=$('uni-grid');
...
_card(u,i){
  ...
  return`<article class="uni-card" onclick="Universities.openModal(${i})">
    <div class="card-logo-area">
      <div class="card-logo-placeholder"><span class="card-logo-text">${escapeHtml(initial)}</span></div>
    </div>
...
openModal(i){
  ...
  $('modal-body').innerHTML=`
    <div class="modal-header">
      <div class="modal-logo-wrap"><span class="modal-logo-text">${escapeHtml(initial)}</span></div>
    ...
    <div class="modal-content">
      <div class="modal-section">
        ...
        <p class="modal-value">${escapeHtml(u.country)}</p>
      </div>
      <div class="modal-section">
        <p class="modal-label">Web Domain(s)</p>
        <p class="modal-value">${escapeHtml(allDomains)}</p>
      </div>
      ...
      <div class="modal-actions">
        ${url?`<a class="modal-apply-btn" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">Visit Official University Website</a>`:`<span style="padding:14px 24px;background:var(--gray-100);color:var(--gray-400);border-radius:8px;text-align:center;font-size:.9rem">No website listed</span>`}
      </div>
    </div>`;
  openModal();
}};
