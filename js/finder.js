...
const Finder = {
  ...
  showResults(results, level, subject, region, funding) {
    ...
    if (!results.length) {
      ...
      $('finder-results-grid').innerHTML = results.map(s => Scholarships._card(s)).join('');
    }
  ...
  reset() {
    this.selections = { level: null, subject: null, region: null, funding: null };
    document.querySelectorAll('.finder-opt').forEach(b => b.classList.remove('active'));
    this.showForm();
  },
};
