...
'use strict';
const EXCLUDED=['British Indian Ocean Territory','French Southern Territories','Saint Helena, Ascension and Tristan da Cunha','Mayotte','Reunion','Western Sahara','Canary Islands'];
const Countries={data:[],loaded:false,
async load(){
  if(this.loaded){this.render();return;}
  clearError('country');showLoader('country');
  const cached=Cache.get('africa_countries');
  ...
  try{
    ...
    const filtered=json.filter(c=>!EXCLUDED.includes(c.name.common)).sort((a,b)=>a.name.common.localeCompare(b.name.common));
    Cache.set('africa_countries',filtered);
...
_card(c){
  ...
  const flagHtml=c.flags?.png?`<img class="country-flag" src="${escapeHtml(c.flags.png)}" alt="Flag of ${escapeHtml(c.name.common)}" loading="lazy"/>`:`<span style="font-size:2.2rem">🏳️</span>`;
  return`<article class="country-card" onclick="Countries._onClick('${escapeHtml(c.name.common).replace(/'/g,"\\'")}',this)" tabindex="0" onkeydown="if(event.key==='Enter')Countries._onClick('${escapeHtml(c.name.common).replace(/'/g,"\\'")}',this)">
    ${flagHtml}
    <p class="country-name">${escapeHtml(c.name.common)}</p>
    <p class="country-capital"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg>${escapeHtml(cap)}</p>
    <p class="country-pop"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>${formatNumber(c.population)}</p>
    ${c.subregion?`<span class="tag tag-gold" style="width:fit-content">${escapeHtml(c.subregion)}</span>`:''}
    <div class="country-cta">🎓 Find universities <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
  </article>`;
},
_onClick(name){
  switchTab('universities');window._closeSidebar?.();
  ...
  sel.value=opt.value;
  Universities.data=[];Universities.loaded=false;Universities.load();
}};
