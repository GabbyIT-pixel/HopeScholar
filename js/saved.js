/* saved.js — HopeScholar | localStorage bookmarks */
'use strict';
const SK = 'hs_saved_v2';
const Saved = {
  items: {},
  init() { try { const r = localStorage.getItem(SK); this.items = r ? JSON.parse(r) : {}; } catch { this.items = {}; } this._badge(); },
  has(id) { return !!this.items[id]; },
  toggle(id, data, type) {
    if (this.items[id]) { delete this.items[id]; this._btn(id, false); showToast('Removed from saved'); }
    else { this.items[id] = { data, type }; this._btn(id, true); showToast('Saved! View in Saved tab '); }
    this._save(); this._badge();
    if ($('tab-saved')?.classList.contains('active')) this.renderTab();
  },
  remove(id, card) { delete this.items[id]; this._save(); this._badge(); if (card) { card.style.transition = 'opacity .2s,transform .2s'; card.style.opacity = '0'; card.style.transform = 'scale(.96)'; setTimeout(() => this.renderTab(), 220); } else this.renderTab(); },
  renderTab() {
    const grid = $('saved-grid'), empty = $('saved-empty'), keys = Object.keys(this.items);
    if (!keys.length) { grid.innerHTML = ''; showEl(empty); return; }
    hideEl(empty);
    grid.innerHTML = keys.map(id => { const { data, type } = this.items[id]; return type === 'scholarship' ? this._scholCard(id, data) : this._uniCard(id, data); }).join('');
  },
  _scholCard(id, s) {
    const catClass = { mastercard: 'tag-red', fullyfunded: 'tag-green', global: 'tag-blue' }[s.category] || 'tag-gray';
    const logoHtml = s.logo ? `<img class="card-logo" src="${escapeHtml(s.logo)}" alt="${escapeHtml(s.shortName || s.name)}" onerror="this.style.display='none'" loading="lazy"/><div class="card-logo-placeholder" style="display:none"><span class="card-logo-text">${escapeHtml((s.shortName || s.name).slice(0, 3))}</span></div>` : `<div class="card-logo-placeholder"><span class="card-logo-text">${escapeHtml((s.shortName || s.name).slice(0, 3))}</span></div>`;
  ...
  _uniCard(id, u) {
    const initial = (u.name || '').split(' ').slice(0, 2).map(w => w[0] || '').join('').toUpperCase().slice(0, 3);
  ...
  _save() { try { localStorage.setItem(SK, JSON.stringify(this.items)); } catch { } },
  _badge() { const b = $('saved-badge'); if (!b) return; const c = Object.keys(this.items).length; b.textContent = c; b.style.display = c > 0 ? 'inline-block' : 'none'; },
  _btn(id, saved) { document.querySelectorAll('.bm-btn').forEach(b => { if ((b.getAttribute('onclick') || '').includes(id)) { b.classList.toggle('saved', saved); b.querySelector('svg')?.setAttribute('fill', saved ? 'currentColor' : 'none'); } }); }
};
