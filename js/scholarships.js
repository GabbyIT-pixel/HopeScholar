...
const Scholarships = {
  render() {
    ...
    if (search)
      list = list.filter(
        (s) =>
          ...
          s.focus.join(" ").toLowerCase().includes(search) ||
    ...
    showStats(
      "schol",
      `${list.length} scholarship${list.length !== 1 ? "s" : ""} found`,
    );
    const nc = $("schol-nav-count");
  ...
  _card(s) {
    ...
    const catLabel =
      {
        mastercard: "Mastercard Foundation",
  ...
  openModal(id) {
    ...
    const logoHtml = s.logo
      ? `<img src="${escapeHtml(s.logo)}" alt="${escapeHtml(s.name)}" style="max-width:52px;max-height:52px;object-fit:contain" onerror="this.style.display='none'">`
      : `<span class="modal-logo-text">${escapeHtml(s.shortName.slice(0, 3))}</span>`;
    const focusPills = s.focus
      .map((f) => `<span class="focus-pill">${escapeHtml(f)}</span>`)
      .join("");
    const tagsPills = s.tags
      .map((t) => `<span class="tag tag-gray">${escapeHtml(t)}</span>`)
      .join("");
...
document.addEventListener("DOMContentLoaded", () => {
  $("schol-search")?.addEventListener(
    "input",
    debounce(() => Scholarships.render(), 220),
  );
